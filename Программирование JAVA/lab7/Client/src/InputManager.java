...
public class InputManager {
    ...
    public void execute() throws IOException, IncorrectDataException, EOFException, ClassNotFoundException {
        ...
        while (statusInput) {
            ...
            command = (line.trim() + " " + " ").split(" ", 4);
            name = command[0].trim();
            arg = command[1].trim();
            end = command[2].trim();
            if (user != null && (name.equals("login")||name.equals("reg")) && arg.equals("")) {
            ...
            switch (name) {
                ...
                case "update":
                    if (user != null) {
                        ...
                        System.out.println("Для того чтобы использовать команды необходимо авторизоваться по команде: \"login\"" +
                                "или зарегистрироваться по команде \"reg\"");
                        continue;
                    }
                    break;
                case "remove_greater":
